...
const surveyData = {
  ...
  questions: [
    ...
    },
    {
      ...
      type: "radio",
      responses: [
        { value: "Required for major", count: 14 },
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Distribution by major</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={surveyData.demographics.major}
                          cx="50%"
                          cy="50%"
                          labelLine={false}
                          outerRadius={80}
                          ...
                          label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}
                        >
                          {surveyData.demographics.major.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip />
